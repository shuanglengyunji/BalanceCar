...
替换 void Speed_OutPut(s32 Speed_Left,s32 Speed_Right) Movement_Control.c 里的 ABS 函数

Balance_Control函数增加输出前限幅

pid初值改为 kp = 20 , kd = 1


2016.12.11
1.质疑编码器输出是否有负值，质疑编码器输出是不是应该把两根线都接上
2.把编码器输出变量从u16改为s32

1.完成编码器部分的部分修正（待测试）
2.完成脉冲-速度转换计算（待测试）
3.完成Balance和Speed两个控制函数的初版编写
4.初步联通整个系统
5.修正ZET6工程的引用路径
...
12.14
1.使用野火提供的PWM输出函数，占用定时器Tim3
2.临时取消 Time_Counter_Config();//微妙计时器初始化
  因为Tim3被占用
3.pid算法初步完成
...
1.PA0被编码器输入占用，key功能关闭

2.18

1.将USART1和USART2的发送函数独立
2.printf同时支持两个串口
